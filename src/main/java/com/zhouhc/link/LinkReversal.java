...
public class LinkReversal {
    ...
    private static void reversalGrouop(ListNode cursorHeadNode, ListNode cursorTailNode) {
        //翻转的元素,初始化为原来头节点
        ListNode nextNode = cursorHeadNode;
        //小组游标遍历的节点,用于不断指向翻转元素的下一个节点的
        ListNode cursorIndexNode = cursorHeadNode;
        //保存翻转前的头节点，其实也是翻转后的尾结点
        ListNode oriHeadNode = cursorHeadNode;
        //翻转的终止条件
        while (nextNode != cursorTailNode) {
            //获取翻转元素的下一个节点
            nextNode = cursorIndexNode.next;
            //保证 cursorHeadNode.next 指向翻转节点的下一个节点
            cursorIndexNode.next = nextNode.next;
            //翻转节点指向头节点
            nextNode.next = cursorHeadNode;
            //保证保证头节点始终是最新的元素
            cursorHeadNode = nextNode;
        }
    }
